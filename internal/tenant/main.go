...
func main() {
	...
	if _, err := mytrace.InitOpenTelemetry(ctx, &cfg.Jaeger, &cfg.ServiceInfo); err != nil {
		logger.Error("Failed to init open telemetry", "err", err.Error())
		return
	}
	...
	// --- 初始化数据库连接 ---
	dsn := fmt.Sprintf("%s:%s@tcp(%s:%d)/%s?%s", cfg.Mysql.Username, cfg.Mysql.Password, cfg.Mysql.Host, cfg.Mysql.Port, cfg.Mysql.DbName, cfg.Mysql.Options)
	db, err := gorm.Open(mysql.Open(dsn), &gorm.Config{})
	if err != nil {
		logger.Error("连接数据库失败", "err", err.Error())
		return
	}
	...
	tenantpb.RegisterTenantServiceServer(gServer, handler)
